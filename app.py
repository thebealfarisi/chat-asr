...
##Buat Widget##
@app.route('/bubble-chat')
def bubble_chat():
    return send_from_directory(app.static_folder,'bubble-chat.html')
...
def firstCall():
    try:
        ...
        with lock:
            ...
            # encoded_audio = createOpenAI(data.get('firstMessage'))
            encoded_audio = createElevenlabs(data.get('firstMessage'))
            response_data = {
                    # 'data': response,
                    'audio': encoded_audio,
                    'isAnswered': True,
